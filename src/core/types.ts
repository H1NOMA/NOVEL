...
export interface Planet {
  ...
  supplied: boolean;
  /**
   * Мир отрезан от ядра территории владельца.
   *
   * Почти для всех это то же самое, что `!supplied`. Разница — у иллюминатов:
   * их владения держатся не на караванах, а на переходах Бездны, перерезать
   * которые нельзя. Их отрезанный мир снабжается по-прежнему, но дерётся
   * хуже: связь с роем разумов ослабла.
   */
  cutOff?: boolean;
  /**
   * Точка людского ресурса иллюминатов: мир, взятый после варп-вторжения.
   * Его население ежедневно пополняет Безмозглые массы.
   */
  harvest?: boolean;
  /** Маяк варп-вторжения: сюда прыгнул флот иллюминатов и ещё не взял мир. */
  warpBeacon?: boolean;
