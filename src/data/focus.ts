...
const superEarth: FocusNode[] = [
  ...
  { id: 'se_orbital', faction: 'superEarth', title: 'Орбитальное превосходство', desc: 'Точные орбитальные орудия сравнивают позиции врага с землёй.', cost: 60, x: 1, y: 3, requires: ['se_stratagems', 'se_eagle', 'se_fleet'], effects: [{ kind: 'combat', amount: 0.2 }] },
  ...
  { id: 'se_blockade', faction: 'superEarth', title: 'Орбитальные блокады', desc: 'Дозорные заслоны над каждым приграничным миром замедляют высадки врага.', cost: 55, x: 2, y: 4, requires: ['se_battlegroup', 'se_warbonds'], effects: [{ kind: 'fortify', amount: 1 }, { kind: 'combat', amount: 0.08 }] },
...
const automatons: FocusNode[] = [
  ...
  { id: 'aut_prime_fleet', faction: 'automatons', title: 'Флагманы Прайма', desc: 'Со стапелей сходит эскадра тяжёлых корпусов — стальной кулак РАЗУМ-9.', cost: 75, x: 2, y: 4, requires: ['aut_stardestroyer', 'aut_forges'], effects: [{ kind: 'heavyFleet', ships: 4, dreadnoughts: 2, battleships: 2, infantry: 50 }] },
  { id: 'aut_iron_horde', faction: 'automatons', title: 'Стальная орда', desc: 'Резервные легионы пробуждаются в законсервированных ангарах.', cost: 70, x: 0, y: 5, requires: ['aut_jets', 'aut_assembly'], effects: [{ kind: 'heavyFleet', ships: 8, dreadnoughts: 3, battleships: 0, infantry: 40 }] },
  ...
  { id: 'aut_watchnet', faction: 'automatons', title: 'Сеть слежения РАЗУМ', desc: 'Миллиард глаз-сенсоров раскрывает каждое движение органиков.', cost: 60, x: 3, y: 6, requires: ['aut_sp_gloomburn', 'aut_dev_ai'], effects: [{ kind: 'revealAll', days: 45 }] },
...
const illuminate: FocusNode[] = [
  ...
  { id: 'ill_dawn_armada', faction: 'illuminate', title: 'Армада рассвета', desc: 'Из-за грани реальности выходит эскадра, что помнит Первую войну.', cost: 75, x: 2, y: 4, requires: ['ill_monolith', 'ill_darkenergy'], effects: [{ kind: 'heavyFleet', ships: 5, dreadnoughts: 2, battleships: 2, infantry: 40 }] },
  { id: 'ill_void_veterans', faction: 'illuminate', title: 'Избранники Пустоты', desc: 'Экипажи, прошедшие сквозь ничто, не знают страха.', cost: 60, x: 2, y: 5, requires: ['ill_dawn_armada', 'ill_leviathans'], effects: [{ kind: 'xpAll', amount: 45 }] },
  { id: 'ill_mind_harvest', faction: 'illuminate', title: 'Жатва разумов', desc: 'Миллионы обращённых пополняют Безголосых. Их воля — наша.', cost: 65, x: 3, y: 6, requires: ['ill_sp_abyss', 'ill_conversion'], effects: [{ kind: 'manpower', amount: 120 }, { kind: 'politicalPower', amount: 80 }] },
  { id: 'ill_shift', faction: 'illuminate', title: 'Пространственный сдвиг', desc: 'Весь флот исчезает — и мгновенно возникает над святилищем.', cost: 70, x: 4, y: 6, requires: ['ill_sp_gloompierce', 'ill_rend'], effects: [{ kind: 'recallFleets' }, { kind: 'xpAll', amount: 30 }] },
...
const terminids: FocusNode[] = [
  ...
  { id: 'term_royal_brood', faction: 'terminids', title: 'Королевские выводки', desc: 'Из глубинных гнёзд поднимаются исполины, рождённые для пустоты.', cost: 75, x: 0, y: 5, requires: ['term_behemoths', 'term_titans'], effects: [{ kind: 'heavyFleet', ships: 6, dreadnoughts: 3, battleships: 1, infantry: 50 }] },
  ...
  { id: 'term_instinct', faction: 'terminids', title: 'Инстинкт улья', desc: 'Каждая смерть учит весь рой сразу. Ошибки не повторяются.', cost: 60, x: 3, y: 6, requires: ['term_dev_apex', 'term_adaptation'], effects: [{ kind: 'xpAll', amount: 45 }, { kind: 'combat', amount: 0.05 }] },
  { id: 'term_spore_tide', faction: 'terminids', title: 'Споровый прилив', desc: 'Все зреющие тучи Мрака сгущаются РАЗОМ. Небо гаснет.', cost: 70, x: 4, y: 6, requires: ['term_sp_deepgloom', 'term_gloom'], effects: [{ kind: 'gloomSurge' }, { kind: 'production', amount: 60 }] },
  { id: 'term_great_spawn', faction: 'terminids', title: 'Великий нерест', desc: 'Ульи содрогаются: рождается больше, чем гибнет за целую войну.', cost: 70, x: 5, y: 5, requires: ['term_broodmothers', 'term_hivemind'], effects: [{ kind: 'manpower', amount: 150 }, { kind: 'garrisonAll', amount: 15 }] },
  { id: 'term_torpor', faction: 'terminids', title: 'Оцепенение роя', desc: 'Рой замирает, переваривая съеденное. Галактика затаила дыхание.', cost: 55, x: 5, y: 6, requires: ['term_great_spawn', 'term_noqueen'], effects: [{ kind: 'truceAll', days: 50 }] },
  { id: 'term_chitin', faction: 'terminids', title: 'Хитиновые бастионы', desc: 'Панцири павших титанов срастаются в живые стены.', cost: 65, x: 6, y: 5, requires: ['term_ter_tunnels', 'term_carapace'], effects: [{ kind: 'fortifyAll', amount: 1 }, { kind: 'garrisonAll', amount: 10 }] },
  ...
  { id: 'term_quiet_spores', faction: 'terminids', title: 'Тихие споры', desc: 'Невидимые споры оседают на чужих мирах и слушают. Рой знает всё.', cost: 60, x: 7, y: 5, requires: ['term_ter_infest', 'term_spores'], effects: [{ kind: 'revealAll', days: 45 }] },
];
